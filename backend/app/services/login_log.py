...
def _trim(session: Session) -> None:
    ids = session.exec(select(LoginAttempt.id).order_by(col(LoginAttempt.id).desc()).offset(MAX_ENTRIES)).all()
    if not ids:
        return
    for row in session.exec(select(LoginAttempt).where(col(LoginAttempt.id).in_(ids))).all():
        session.delete(row)
    session.commit()


def recent_attempts(session: Session, limit: int = 20) -> list[LoginAttempt]:
    return list(session.exec(select(LoginAttempt).order_by(col(LoginAttempt.id).desc()).limit(limit)).all())
