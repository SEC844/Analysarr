...
- **Réparation de hardlink** : action manuelle (fiche média, bouton "Réparer les hardlinks", preview puis confirmation) pour les torrents `is_hardlinked = false` ET `repairable = true`. Entièrement automatique une fois confirmé, en équivalent de `ln -f SOURCE CIBLE` (le fichier CIBLE est remplacé par un hardlink vers SOURCE) — un seul clic répare autant de fichiers que nécessaire (un épisode, une saison entière, ou toute la série). Deux sens possibles par paire fichier-bibliothèque/fichier-torrent, selon qu'un hardlink valide existe déjà quelque part pour ce fichier :
  ...
  - Détection "déjà protégé" **par fichier**, jamais par torrent seul : `Torrent.inode`/`Torrent.device` (colonnes DB, calculées au scan) ne retiennent qu'UN SEUL fichier représentatif par torrent (le premier résolu, voir `scan.py`) — insuffisant pour juger un torrent multi-fichiers (pack saison) "déjà protégé" dans son ensemble. `build_repair_preview` résout donc en direct, à chaque appel, l'inode de CHAQUE fichier de CHAQUE torrent déjà `is_hardlinked = true` du média, pas seulement la valeur DB. Bug réel corrigé après test en conditions réelles (série multi-épisodes, pack saison déjà protégé) : sans ce correctif, seul le premier épisode du pack était reconnu comme protégé, et les autres épisodes se voyaient proposer à tort la direction `torrent_to_library` — qui aurait remplacé leur hardlink déjà fonctionnel par un lien vers un torrent différent (potentiellement deux fois de suite si deux torrents orphelins différents proposaient la même cible).
  - Sécurité critique (les deux sens) : le nouveau lien est toujours créé à côté de la CIBLE (`<chemin>.analysarr-tmp`) puis substitué par `os.replace()` (atomique) — l'original n'est **jamais** supprimé avant que le nouveau lien n'existe. Si `os.link()` échoue avec `EXDEV`/Errno 18 (torrent et bibliothèque RÉELLEMENT sur des systèmes de fichiers différents — arrive typiquement quand un dossier `cross-seed` dédié n'est pas monté sur le même disque que le reste ; un hardlink est par nature impossible entre deux systèmes de fichiers, ce n'est pas contournable), `_relink` retente avec `os.symlink()` : un lien symbolique traverse les points de montage sans coût d'espace disque. `stat_inode()` (hardlink.py) utilise partout `os.stat()`, jamais `os.lstat()` — un lien symbolique est donc vu exactement comme un hardlink par tout le reste de l'app (même `(inode, device)` que le fichier cible réel). Avant de substituer la cible, on vérifie que le lien symbolique résout bien vers un fichier réel DEPUIS LE CONTENEUR ANALYSARR (`os.path.exists`, qui suit les liens) ; sinon rien n'est substitué, l'erreur d'origine est remontée telle quelle. Contrepartie explicite (affichée sur l'étape concernée, `used_symlink = true`) : le conteneur qui lit ce chemin ensuite (Emby pour la bibliothèque, qBittorrent pour un torrent) doit pouvoir résoudre `source_path` lui-même — même prérequis de montages identiques déjà nécessaire pour qu'un hardlink fonctionne, donc pas un risque nouveau, mais à vérifier visuellement après une réparation par repli symbolique.
  - Aucune vérification `st_dev` préalable pour écarter un candidat de la preview : sur certains montages virtualisés (`shfs` d'Unraid notamment), le `st_dev` rapporté peut être incohérent selon le chemin de montage utilisé pour atteindre un même fichier physique — deviner à l'avance si un hardlink va réussir n'est pas fiable. Chaque candidat au contenu vérifié (même taille) est donc proposé, et c'est la tentative réelle qui tranche (hardlink, puis repli symbolique si EXDEV) ; un échec réel ne casse rien (voir sécurité critique ci-dessus), il ne fait que s'afficher comme tel sur l'étape concernée dans le résultat.
- **Statuts** (cumulables, un média peut en porter plusieurs à la fois) :
  ...
  - `manquant_emby` — le média n'a pas été retrouvé dans Emby, OU (séries) certains épisodes que Sonarr a téléchargés (`episodeFile` existant) n'ont pas été repris par Emby (import manqué sur CES épisodes-là seulement — la série elle-même EST bien dans Emby, `emby_item_id` renseigné). Ce deuxième cas était invisible avant : la boucle de construction des `MediaFile` ne parcourt que ce qu'Emby renvoie (`emby.get_episodes`), donc un épisode absent d'Emby ne produit tout simplement aucun `MediaFile` — sans comparaison explicite contre la liste d'épisodes de Sonarr (`sonarr.get_episodes`, `hasFile`), rien ne signalait cette absence. `Media.missing_emby_episodes` (labels `SxxEyy` séparés par des virgules) porte le détail, affiché sur la fiche média.
...
- **Emby** : API REST classique (`/Items`, `/Items/{Id}/Images/Primary` pour les jaquettes). Jaquettes mises en cache disque (`services/poster_cache.py`, dossier `posters/` à côté de la base SQLite — même volume Docker persistant) plutôt que retéléchargées à chaque affichage. Clé de cache : `(emby_item_id, poster_image_tag)` — `poster_image_tag` vient d'`ImageTags.Primary`, capturé à chaque scan (champ `Media.poster_image_tag`) ; un tag différent (jaquette changée côté Emby) ne matche jamais un fichier existant, donc jamais de contenu périmé servi, et l'ancienne version du même item est supprimée à l'écriture. Le frontend appelle `GET /api/media/{id}/poster?v=<tag>` (`lib/api.ts::posterUrl`) : l'URL change automatiquement quand la jaquette change, donc le navigateur peut la mettre en cache indéfiniment (`Cache-Control: immutable`) sans jamais risquer d'afficher une version périmée.
